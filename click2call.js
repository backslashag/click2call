...
var phonenumber = {
	...
	_getChildTextNodes: function(node) {
		...
		for (var i = 0; i < node.childNodes.length; i++) {
			...
			var currentNodeName = child.nodeName.toLowerCase();
			if (currentNodeName === "script" || currentNodeName === "a") {
				// we are not interested in script-parts and links
				continue;
			} else if (child.nodeType === 3 && child.nodeValue.match(/\d+/) ) {
				// text node which contains some digits
				nodes.push({node: child, parent: node});
			} else if (child.nodeType === 1) {
				// has subnodes, let's go deeper
